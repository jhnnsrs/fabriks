...
from fabriks.stores.protocol import FabriksStore, validate_relative


def put_bytes(store: FabriksStore, path: str, data: bytes) -> None:
    """Write bytes to a store, whatever flavour of store it is."""
    store.put(validate_relative(path), data)


def get_bytes(store: FabriksStore, path: str) -> bytes:
...
def _payload(result: Any, store: FabriksStore, path: str) -> bytes:  # noqa: ANN401
    ...
    try:
        ...
        raise FormatError(
            ...
            f".bytes() or .read() method and is not a buffer, so fabriks cannot read {path!r} from it."
        ) from error


def get_range_bytes(store: FabriksStore, path: str, start: int, length: int) -> bytes:
...
async def aget_bytes(store: FabriksStore, path: str) -> bytes:
...
async def aget_range_bytes(store: FabriksStore, path: str, start: int, length: int) -> bytes:
...
def list_paths(store: FabriksStore, prefix: str = "") -> list[str]:
    ...
    def absorb(item: Any) -> None:  # noqa: ANN401
        ...
        if isinstance(item, Iterable):
            ...
            return
        raise FormatError(
            f"{type(store).__name__}.list yielded {type(item).__name__}, which fabriks cannot read a path out of."
        )
