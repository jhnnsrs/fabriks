"""What fabriks asks of a store, and the path rules every one of them is held to.

Three protocols, none of which anything inherits from: what a store has to offer, and the two
things it may offer that fabriks will use if they are there. Keeping the optional halves in
...
The implementations that ship with fabriks are :mod:`fabriks.stores.directory` and
:mod:`fabriks.stores.memory`; the helpers that call through these methods are in
:mod:`fabriks.stores.access`.
"""
...
@runtime_checkable
class FabriksStore(Protocol):
    """The three operations fabriks needs from a store.
...
class RangeReadable(Protocol):
    ...
    Kept separate from :class:`FabriksStore` so that a store without it still satisfies the
    protocol fabriks requires -- ``isinstance(store, RangeReadable)`` is then the question
    :func:`fabriks.stores.access.get_range_bytes` asks before deciding whether to fetch a window
    or the whole object.
    """
