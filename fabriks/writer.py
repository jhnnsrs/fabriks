...
The same tree lands on a local directory and in an S3 prefix; see :mod:`fabriks.stores`.
"""
...
from fabriks.build import MeshCollection, build_collection
...
from fabriks.sources import MeshSource
from fabriks.stores import FabriksStore, join, put_bytes
...
def _plan_parts(shard: pa.Table, max_part_bytes: int) -> list[pa.Table]:
    ...
    :func:`fabriks.frames.plan_byte_chunks`, which both go through.
    """
    if shard.num_rows == 0:
        return [shard]
...
def _with_locators(catalog: pa.Table, locators: Mapping[tuple[int, int], tuple[int, int, int]]) -> pa.Table:
    ...
    a catalog row without a locator is a real inconsistency, and :func:`fabriks.verify` should
...
def write_meshes(
    ...
    *,
    ...
    The convenience form of :func:`fabriks.build_collection` followed by
    ...
    levels are made; see :func:`fabriks.build_collection`.
    """
