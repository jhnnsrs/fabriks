...
:func:`fabriks.write_collection` fills them in as it lands each part.
"""
...
from fabriks.codecs import QUANT_MAX, encode_indices, encode_positions
from fabriks.errors import FormatError
from fabriks.frames import arrow_schemas, build_table
from fabriks.geometry import (
    clip_to_cells,
...
from fabriks.simplifiers import Simplifier, resolve_simplifier, simplify_to_target
...
class MeshCollection:
    ...
    def write(self, store: FabriksStore, prefix: str = "") -> Manifest:
        """Write this collection into a store. See :func:`fabriks.write_collection`."""
...
def build_collection(
    ...
    **Components are positional throughout, and never decoded through a name.** Vertex
    ...
    ``"GREEDY"``. Pass an instance instead -- ``fabriks.GreedyEdgeCollapse(placement="onto_fixed")``
    -- to adjust a backend's own settings, or your own object providing ``simplify``. See
    :mod:`fabriks.simplifiers`. ``decimation`` is how much survives each level,
    defaulting to :meth:`fabriks.Decimation.quarter` -- and whatever it is, the manifest declares
