...
below it is defined by :mod:`fabriks.codecs` and nowhere else.
...
from fabriks.errors import FormatError, MissingExtraError
...
def require_pyarrow() -> Any:  # noqa: ANN401
    ...
    try:
        ...
        raise MissingExtraError(
            "pyarrow is required: a mesh collection is Parquet. Install it with `pip install fabriks`."
        ) from error
    return pyarrow
...
def validate_columns(table: Any, role: str) -> None:  # noqa: ANN401
    ...
    if missing:
        raise FormatError(
            ...
            f"{', '.join(sorted(names))}). Build it with `fabriks.build_collection`, which produces all three frames "
...
def _column_names(table: Any) -> Iterable[str]:  # noqa: ANN401
    ...
    raise FormatError(f"fabriks cannot read column names off a {type(table).__name__}.")
