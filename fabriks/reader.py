...
through :class:`fabriks.stores.StoreFile` -- a seekable view over the store -- so pyarrow reads
...
def _present(
    ...
    server checks, so a null here is a catalog fabriks did not write. Saying which column and
    ...
    if value is None:
        raise FormatError(
            ...
            f"A catalog with a null there was not written by fabriks, and reading it would silently "
...
class Collection:
    ...
    def __init__(self, store: FabriksStore, prefix: str = "") -> None:
    ...
    def _part_files(self) -> dict[tuple[int, int], StoreFile]:
        """The :class:`~fabriks.stores.StoreFile` under each open part, for the async path.
    ...
    def _decode(self, entry: CellEntry, table: pa.Table) -> DecodedCell:
        ...
        try:
            ...
            raise FormatError(
                ...
                f"`fabriks.verify` reports which."
            ) from error
        ...
        # them. See `fabriks.manifest.Encoding`.
...
def open_collection(store: FabriksStore, prefix: str = "") -> Collection:
...
async def aopen_collection(store: FabriksStore, prefix: str = "") -> Collection:
...
async def _amanifest_bytes(store: FabriksStore, prefix: str) -> bytes:
    """Fetch ``fabriks.json``, naming a missing one as the unfinished write it is."""
