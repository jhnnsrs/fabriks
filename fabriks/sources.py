...
trimesh is a dependency of fabriks rather than an extra -- ``slice_mesh_plane`` is how a mesh is
cut at the cell planes, and it is not a function worth reimplementing -- so it is imported
plainly here. What the second shape buys is that the dependency stays *fabriks's*: a caller with
...
def coerce_mesh(source: MeshSource) -> Mesh:
    ...
    if vertices is None or faces is None:
        if isinstance(source, Sequence) and len(source) == 2:
            ...
            raise TypeError(
                f"An object is a trimesh.Trimesh, a fabriks.Mesh, or a (vertices, faces) pair; got "
