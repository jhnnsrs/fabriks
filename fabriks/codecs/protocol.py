...
:func:`fabriks.codecs.blobs.codec_for` resolves that name to an implementation. Adding one means
a module here and an entry in that table, and nothing above the package changes.

The two that ship are :mod:`fabriks.codecs.raw` (``NONE``, the format's default -- a blob is the
renderer's buffer verbatim) and :mod:`fabriks.codecs.meshopt` (``MESHOPT``).
"""
