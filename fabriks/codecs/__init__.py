...
Where each part is
------------------
:mod:`~fabriks.codecs.blobs` is the quantization either side of a codec, and the dispatch from a
manifest's ``codec`` value to an implementation. :mod:`~fabriks.codecs.protocol` states what an
implementation is; :mod:`~fabriks.codecs.raw` and :mod:`~fabriks.codecs.meshopt` are the two that
ship, and :mod:`~fabriks.codecs.compression` is the blob compression declared alongside them.

How space is divided -- the octree levels, the Morton code a ``cell`` is, and the box it stands
for -- is :mod:`fabriks.octree`, not this package. Quantization needs a cell's box and imports it
...
:mod:`~fabriks.codecs.blobs` -- not as a new field.
...
from fabriks.codecs.blobs import (
    QUANT_MAX,
    codec_for,
    decode_indices,
    decode_positions,
...
from fabriks.codecs.compression import compress, decompress, require_known_compression
from fabriks.codecs.meshopt import MeshoptCodec, require_meshoptimizer
from fabriks.codecs.protocol import BlobCodec
from fabriks.codecs.raw import RawCodec
