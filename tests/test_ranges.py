...
def wide_objects() -> dict[int, Any]:
    ...
    :attr:`fabriks.StoreFile.TAIL_BYTES`* -- so opening a part pulls the whole thing as its tail
...
@pytest.fixture(scope="session")
def wide(wide_objects: dict[int, Any]) -> fabriks.MeshCollection:
    """The larger collection, built once for the whole session."""
    with warnings.catch_warnings():
        warnings.simplefilter("ignore")
        return fabriks.build_collection(wide_objects, cell_size=CELL_SIZE, levels=LEVELS)
...
@pytest.fixture()
def measured(chunked: AccountingStore) -> fabriks.Collection:
    """The chunked collection, opened, with the catalogs already read and the record cleared."""
    opened = fabriks.open_collection(chunked, "c")
...
def test_the_level_actually_splits_into_row_groups(chunked: AccountingStore):
    ...
    manifest = fabriks.open_collection(chunked, "c").manifest
...
def test_reading_a_cell_costs_about_one_row_group(
    chunked: AccountingStore, measured: fabriks.Collection
):
...
def test_reading_a_plan_shares_a_row_group_between_the_cells_in_it(
    ...
    one_at_a_time = fabriks.open_collection(chunked, "c")
...
def test_read_cells_yields_in_the_order_it_was_asked_for(measured: fabriks.Collection):
...
def test_every_cell_with_geometry_is_located(measured: fabriks.Collection):
...
def test_every_cell_reads_back_as_the_size_its_catalog_row_claims(measured: fabriks.Collection):
...
def test_a_locator_pointing_at_the_wrong_row_group_is_caught(chunked: AccountingStore):
    ...
    with pytest.raises(fabriks.FormatError, match="catalog and the geometry disagree"):
        opened.read_cell(entry.level, entry.cell)


def test_blob_bytes_is_what_the_cell_actually_carries(measured: fabriks.Collection):
...
def test_a_collection_whose_manifest_records_no_length_still_reads(
    ...
    -- which is what fabriks did everywhere before locators existed.
    """
...
def test_a_store_that_cannot_range_read_still_reads(collection: fabriks.MeshCollection):
    """``get_range`` is optional, and its absence degrades rather than fails."""

    class NoRanges:
        """A store offering only the three methods fabriks requires, and not one more."""

        def __init__(self) -> None:
            self.inner = fabriks.MemoryStore()
...
def test_releasing_drops_the_geometry_and_keeps_the_catalogs(measured: fabriks.Collection):
...
def test_the_round_trip_is_unchanged_by_how_finely_it_was_chunked(
    ...
    fabriks.write_collection(collection, fine, "c", row_group_bytes=1024)

    one, other = fabriks.open_collection(coarse, "c"), fabriks.open_collection(fine, "c")
