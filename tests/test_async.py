...
class ConcurrentStore(fabriks.MemoryStore):
...
@pytest.fixture(scope="session")
def many(many_objects: dict[int, Any]) -> fabriks.MeshCollection:
    """The larger collection, built once."""
    with warnings.catch_warnings():
        warnings.simplefilter("ignore")
        return fabriks.build_collection(many_objects, cell_size=CELL_SIZE, levels=LEVELS)


@pytest.fixture()
def served(many: fabriks.MeshCollection) -> ConcurrentStore:
    """The collection in a store whose reads are async and observable."""
    store = ConcurrentStore()
    fabriks.write_collection(many, store, "c", row_group_bytes=ROW_GROUP_BYTES)
    return store
...
def test_opening_a_collection_without_blocking_reads_the_same_manifest(served: ConcurrentStore):
    ...
    synchronous = fabriks.open_collection(served, "c")

    asynchronous = asyncio.run(fabriks.aopen_collection(served, "c"))
...
def test_opening_an_interrupted_write_is_refused_asynchronously_too(served: ConcurrentStore):
    ...
    del served.objects["c/fabriks.json"]

    with pytest.raises(fabriks.UnfinishedCollectionError, match="manifest last"):
        asyncio.run(fabriks.aopen_collection(served, "c"))
...
def test_the_async_reader_returns_exactly_what_the_sync_one_does(served: ConcurrentStore):
    ...
    reference = fabriks.open_collection(served, "c")
...
def test_the_reads_actually_overlap(served: ConcurrentStore):
    ...
    collection = fabriks.open_collection(served, "c")
...
def test_the_concurrency_bound_is_respected(served: ConcurrentStore):
    """A viewer must be able to stop fabriks from opening a hundred sockets at once."""
    collection = fabriks.open_collection(served, "c")
    keys = [(entry.level, entry.cell) for entry in collection.plan(error_budget=0.3)]
...
def test_a_store_with_no_async_methods_still_reads(many: fabriks.MeshCollection):
    """Async is optional on a store, so its absence runs the sync path in a thread."""
    store = fabriks.MemoryStore()
    fabriks.write_collection(many, store, "c", row_group_bytes=ROW_GROUP_BYTES)
    reference = fabriks.open_collection(store, "c")
    keys = [(entry.level, entry.cell) for entry in reference.plan(error_budget=0.3)]
...
def test_two_overlapping_reads_open_each_part_once(served: ConcurrentStore):
    ...
    async def both() -> tuple[list[fabriks.DecodedCell], int, int]:
        collection = await fabriks.aopen_collection(served, "c")
...
def test_the_native_async_methods_are_the_ones_used(many: fabriks.MeshCollection):
    """obstore has ``get_range_async``; fabriks must call it rather than thread its sync twin."""
    ...
    collection = fabriks.open_collection(store, "c")
    keys = [(entry.level, entry.cell) for entry in collection.plan(error_budget=0.3)]
...
async def _read(
    store: fabriks.FabriksStore, keys: list[tuple[int, int]], *, concurrency: int = 16
) -> list[fabriks.DecodedCell]:
    """Open and read, the way a viewer would."""
    collection = await fabriks.aopen_collection(store, "c")
    return await collection.aread_cells(keys, concurrency=concurrency)


async def _one(store: fabriks.FabriksStore, level: int, cell: int) -> fabriks.DecodedCell:
