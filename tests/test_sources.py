...
import fabriks
from fabriks.sources import Mesh, coerce_mesh, coerce_objects
...
def test_a_fabriks_mesh_passes_through_unchanged():
...
def test_something_that_is_not_a_mesh_at_all_is_refused():
    ...
    with pytest.raises(TypeError, match="trimesh.Trimesh, a fabriks.Mesh, or a"):
        coerce_mesh(42)
...
def test_an_empty_collection_is_refused():
    ...
    with pytest.raises(fabriks.FormatError, match="at least one object"):
        fabriks.build_collection({}, cell_size=(64, 64, 64))


def test_choosing_a_cell_size_for_nothing_is_refused():
    """The heuristic reads the objects, so it cannot answer without any."""
    with pytest.raises(fabriks.FormatError, match="empty collection"):
        fabriks.choose_cell_size({})
...
def test_an_unknown_codec_is_refused_before_the_expensive_work():
    ...
    with pytest.raises(fabriks.FormatError, match="the format defines"):
