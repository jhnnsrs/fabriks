...
@pytest.fixture(scope="session")
def collection(objects: dict[int, Any]) -> fabriks.MeshCollection:
    ...
    with warnings.catch_warnings():
        ...
        return fabriks.build_collection(objects, cell_size=CELL_SIZE, levels=LEVELS)


@pytest.fixture(scope="session")
def written(collection: fabriks.MeshCollection) -> fabriks.MemoryStore:
...
@pytest.fixture()
def opened(written: fabriks.MemoryStore) -> fabriks.Collection:
    """The collection, read back. Fresh per test so the lazy caches are not shared."""
    return fabriks.open_collection(written, "collection")


class AccountingStore(fabriks.MemoryStore):
...
def vertices_of(mesh: Any) -> np.ndarray:  # noqa: ANN401
    """The vertices of a trimesh or a fabriks Mesh, as a float array."""
