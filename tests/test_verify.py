...
import fabriks
from fabriks.frames import parquet_to_table, table_to_parquet
from fabriks.geometry import on_planes
from fabriks.verify import TIERS, verify
from tests.conftest import CELL_SIZE, LEVELS


@pytest.fixture()
def sound(collection: fabriks.MeshCollection) -> fabriks.MemoryStore:
...
def opened(store: fabriks.MemoryStore) -> fabriks.Collection:
    """The collection under test."""
    return fabriks.open_collection(store, "c")


def failed(store: fabriks.MemoryStore, *, tier: str = "blobs") -> set[str]:
    """The names of the checks that did not pass."""
    return {check.name for check in verify(opened(store), tier=tier).failures}


def edit_catalog(store: fabriks.MemoryStore, column: str, values: list[Any]) -> None:
...
def edit_geometry(store: fabriks.MemoryStore, path: str, column: str, values: list[Any]) -> None:
    ...
    manifest = json.loads(store.objects["c/fabriks.json"])
    for entries in manifest["files"]["levels"].values():
        for entry in entries:
            if entry["path"] == path:
                entry["bytes"] = len(body)
                entry["rowGroups"] = 1
    store.objects["c/fabriks.json"] = json.dumps(manifest).encode()


@pytest.mark.parametrize("tier", TIERS)
def test_a_collection_the_writer_wrote_passes_every_tier(sound: fabriks.MemoryStore, tier: str):
...
def test_the_tiers_are_cumulative_and_cost_more_as_they_go(sound: fabriks.MemoryStore):
...
def test_an_unknown_tier_is_refused(sound: fabriks.MemoryStore):
...
def test_a_cleared_child_mask_bit_is_caught(sound: fabriks.MemoryStore):
...
def test_an_inverted_lod_error_is_caught(sound: fabriks.MemoryStore):
...
def test_a_missing_locator_is_caught(sound: fabriks.MemoryStore):
...
def test_a_locator_pointing_at_a_part_that_does_not_exist_is_caught(sound: fabriks.MemoryStore):
...
def test_a_row_group_past_the_end_of_its_part_is_caught(sound: fabriks.MemoryStore):
...
def test_an_object_catalog_naming_a_cell_that_does_not_exist_is_caught(sound: fabriks.MemoryStore):
...
def test_a_wrong_vertex_count_is_caught_when_the_blob_is_decoded(sound: fabriks.MemoryStore):
...
def test_the_geometry_checks_actually_compared_something(sound: fabriks.MemoryStore):
...
def test_a_coarse_level_larger_than_the_one_below_is_caught(sound: fabriks.MemoryStore):
...
def test_a_boundary_vertex_that_moved_is_caught(sound: fabriks.MemoryStore):
    ...
    verifier to notice.
    """
    from fabriks.codecs import decode_positions, encode_positions
    from fabriks.geometry import on_planes
    ...
    for level in range(LEVELS - 1, 0, -1):
        ...
        for row, cell in enumerate(cells):
            origin, extent = fabriks.cell_box(cell, level, collection.grid.cell_size)
            ...
            # positive.
            pinned = on_planes(vertices, extent, tolerance=1.0 / fabriks.QUANT_MAX)
            if not pinned.any():
                continue
...
def test_a_surface_tangent_to_a_cell_plane_is_not_reported_as_drifted():
    ...
    complain, because it asked :func:`~fabriks.geometry.on_planes` a cell-relative question in
    ...
    with warnings.catch_warnings():
        ...
        built = fabriks.build_collection({1: tangent, 2: clear}, cell_size=CELL_SIZE, levels=LEVELS)
    store = fabriks.MemoryStore()
    fabriks.write_collection(built, store, "c")
...
def test_a_single_level_collection_verifies(objects: dict[int, Any]):
    ...
    with warnings.catch_warnings():
        warnings.simplefilter("ignore")
        built = fabriks.build_collection(objects, cell_size=CELL_SIZE, levels=1)
    store = fabriks.MemoryStore()
    fabriks.write_collection(built, store, "c")

    report = verify(fabriks.open_collection(store, "c"), tier="geometry")

    assert report.ok, str(report)


def test_the_manifest_and_the_report_agree_on_which_files_exist(sound: fabriks.MemoryStore):
    """``verify_paths`` is what a caller iterates to copy or check a collection by hand."""
    from fabriks.verify import verify_paths
