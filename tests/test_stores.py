...
The point of the protocol is that fabriks never learns which kind of store it has. So the
...
def test_obstore_satisfies_the_protocol_with_no_adapter():
    ...
    ``LocalStore`` is usable as it is -- which is what lets fabriks reach S3 without depending
    on obstore.
    """
...
def test_the_same_tree_lands_in_every_kind_of_store(
    ...
    fabriks.write_collection(collection, store, "collection")  # type: ignore[arg-type]
...
def test_a_collection_may_not_name_a_file_outside_its_own_tree():
    ...
    with pytest.raises(fabriks.FormatError, match="cannot be absolute"):
...
def test_a_store_whose_get_returns_something_unreadable_says_so():
    ...
    with pytest.raises(fabriks.FormatError, match="is not bytes"):
