...
def test_geometry_round_trips_through_an_anisotropic_grid(
    ...
    for object_id, source in objects.items():
        ...
        quantum = np.asarray(CELL_SIZE, dtype=np.float64) / fabriks.QUANT_MAX
...
def test_the_encoding_always_states_the_keys_a_decoder_cannot_infer(
    ...
    assert encoding["boundary"] == fabriks.BOUNDARY_LOCKED
    assert encoding["compression"] == fabriks.COMPRESSION_NONE, "declared only where it is applied"
...
def _is_child(cell: int, parent: fabriks.CellEntry) -> bool:
    """Whether a finer cell is one of a parent's eight children."""
    i, j, k = fabriks.morton_decode(cell)
...
def test_raw_vertex_and_face_arrays_are_accepted_without_trimesh_in_the_caller(objects: dict):
    ...
    with warnings.catch_warnings():
        warnings.simplefilter("ignore")
        from_pair = fabriks.build_collection(pairs, cell_size=CELL_SIZE, levels=2)
        from_mesh = fabriks.build_collection({3: source}, cell_size=CELL_SIZE, levels=2)
...
def test_faces_that_do_not_belong_to_their_vertices_are_refused():
    ...
    with pytest.raises(ValueError, match="Object 4"):
        fabriks.build_collection(
            {4: (np.zeros((3, 3)), np.array([[0, 1, 9]]))}, cell_size=CELL_SIZE
        )


def test_lod_error_is_per_cell_rather_than_per_level(collection: fabriks.MeshCollection):
...
def test_level_zero_carries_only_the_quantization_error(collection: fabriks.MeshCollection):
    """Nothing is decimated at level 0, so its error is the quantization step and nothing else."""
    catalog = collection.cell_catalog
    expected = max(collection.grid.cell_extent(0)) / fabriks.QUANT_MAX
...
def test_tightening_the_budget_never_coarsens_a_region(collection: fabriks.MeshCollection):
...
def _covers(candidate: tuple[int, int], target: tuple[int, int]) -> bool:
    ...
    i, j, k = fabriks.morton_decode(candidate_cell)
    return (i >> steps, j >> steps, k >> steps) == fabriks.morton_decode(target_cell)


def test_the_component_order_is_the_callers_and_fabriks_never_interprets_it():
    """fabriks addresses components by position, never by name -- so any order works.
    ...
    store = fabriks.MemoryStore()
    fabriks.write_collection(reversed_, store, "c")
    decoded = fabriks.open_collection(store, "c").object_mesh(7)
...
def test_the_component_order_changes_the_partition_and_never_the_geometry():
    ...
    a renderer interprets the collection's relationship to its source -- which is why fabriks
