...
import fabriks
from fabriks.geometry import border_vertices
...
from fabriks.simplifiers import quadric as quadric_module

BACKENDS = (fabriks.SIMPLIFICATION_QUADRIC, fabriks.SIMPLIFICATION_GREEDY)


def backend(name: str) -> fabriks.Simplifier:
...
def test_the_quadric_backend_pins_the_boundary_and_frees_the_interior():
    ...
    result = fabriks.QuadricSimplifier().simplify(
        vertices, faces, fixed=np.zeros(len(vertices), dtype=bool), target_faces=len(faces) // 4
    )
...
def test_the_quadric_backend_falls_back_when_the_boundary_did_not_hold():
    ...
    class NeverHolds(fabriks.QuadricSimplifier):
        pass
    ...
    assert result.backend == fabriks.GreedyEdgeCollapse().name, "the fallback did not take over"
...
def test_a_backend_this_package_does_not_ship_is_refused_by_name():
    ...
    with pytest.raises(fabriks.FormatError, match="`simplification` is"):
        simplifier_for("MESHLAB")

    with pytest.raises(TypeError, match="QUADRIC"):
        resolve_simplifier(object())

    chosen = fabriks.GreedyEdgeCollapse()
    assert resolve_simplifier(chosen) is chosen
    assert isinstance(resolve_simplifier(fabriks.SIMPLIFICATION_GREEDY), fabriks.GreedyEdgeCollapse)
...
def test_a_collection_built_with_either_backend_holds_the_same_invariants(name: str, objects: dict):
    ...
    with warnings.catch_warnings():
        ...
        collection = fabriks.build_collection(
...
def test_the_backend_choice_does_not_change_the_partition(objects: dict):
    ...
    for name in BACKENDS:
        with warnings.catch_warnings():
            warnings.simplefilter("ignore")
            built[name] = fabriks.build_collection(
                objects, cell_size=(128, 128, 64), levels=3, simplifier=backend(name)
            )
    ...
    for level in range(3):
        ...
        assert cells[fabriks.SIMPLIFICATION_QUADRIC] == cells[fabriks.SIMPLIFICATION_GREEDY], (
...
def test_the_manifest_declares_the_reduction_that_was_actually_applied(objects: dict):
    ...
    with warnings.catch_warnings():
        warnings.simplefilter("ignore")
        quarter = fabriks.build_collection(objects, cell_size=(128, 128, 64), levels=2)
        half = fabriks.build_collection(
            objects, cell_size=(128, 128, 64), levels=2, decimation=fabriks.Decimation.half()
        )

    assert quarter.manifest.encoding.decimation == fabriks.DECIMATION_QUARTER
    assert half.manifest.encoding.decimation == fabriks.DECIMATION_HALF
...
def test_a_ratio_and_its_declaration_are_required_to_agree():
    ...
    with pytest.raises(fabriks.FormatError, match="is HALF, not QUARTER"):
        fabriks.Decimation(ratio=0.5)

    assert fabriks.Decimation.custom(0.3).declaration == fabriks.DECIMATION_CUSTOM
    assert fabriks.Decimation.custom(0.25).declaration == fabriks.DECIMATION_QUARTER
    with pytest.raises(fabriks.FormatError, match="between 0 and 1"):
        fabriks.Decimation.custom(1.5)
    with pytest.raises(fabriks.FormatError, match="at least one tetrahedron"):
        fabriks.Decimation(ratio=0.25, floor_faces=2)
...
def test_the_face_floor_is_configurable():
    ...
    schedule = fabriks.Decimation.quarter(floor_faces=32)

    assert schedule.target_faces(1000, 0) == 1000
    assert schedule.target_faces(1000, 3) == 32, "the floor, not 15"
    assert fabriks.Decimation.quarter().target_faces(1000, 3) == 16
