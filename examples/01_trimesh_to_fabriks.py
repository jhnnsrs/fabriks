...
marching cubes over a label volume -- and writes a fabriks collection: an octree of surfaces as
...
def demo_objects(count: int = 32) -> dict[int, trimesh.Trimesh]:
    ...
    A segmentation is what fabriks is for, so the demo scene is shaped like one: a few dozen
...
def store() -> fabriks.DirectoryStore:
    ...
    same tree to S3 -- fabriks asks a store for ``put``/``get``/``list`` and nothing else.
    """
    return fabriks.DirectoryStore(OUTPUT, create=True)
...
def main() -> None:
    ...
    manifest = fabriks.write_meshes(
        ...
        store(),
        prefix=COLLECTION,
        # In voxels, one size per component, in the same order as the vertices. fabriks never
    ...
    print(
        ...
        "atomic 'upload finished' flag, so a tree without `fabriks.json` is an interrupted write\n"
    ...
    collection = fabriks.build_collection(objects, cell_size=CELL_SIZE, levels=LEVELS)
    ...
    fabriks.validate_columns(collection.cell_catalog, "cell_catalog")
    ...
    reversed_collection = fabriks.build_collection(
        same_scene_reversed, cell_size=CELL_SIZE[::-1], levels=1
    )
    forward = fabriks.build_collection(objects, cell_size=CELL_SIZE, levels=1)
    ...
    print(
        "\nfabriks addresses components by position and never by name -- not when quantizing, not\n"
    ...
    print(
        "fabriks asks a store for put/get/list and nothing else, which is deliberately the shape\n"
        "obstore already has. So the same call writes to S3 with no adapter:\n"
        "\n"
        "    from obstore.store import S3Store\n"
        "    fabriks.write_meshes(objects, S3Store('my-bucket', ...), prefix=key, cell_size=...)\n"
        "\n"
        "and `fabriks.MemoryStore()` keeps the whole thing in a dict, which is what the tests use."
    )

    in_memory = fabriks.MemoryStore()
    fabriks.write_collection(collection, in_memory, "in-memory")
