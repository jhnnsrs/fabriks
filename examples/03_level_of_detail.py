...
def ensure_collection() -> fabriks.Collection:
    """Write the demo collection if example 1 has not been run yet, and open it either way."""
    store = fabriks.DirectoryStore(OUTPUT, create=True)
    if not (OUTPUT / COLLECTION / "fabriks.json").is_file():
        print(f"No collection at {OUTPUT / COLLECTION}, writing one first...")
        fabriks.write_meshes(demo_objects(), store, prefix=COLLECTION, cell_size=CELL_SIZE, levels=LEVELS)

    return fabriks.open_collection(store, COLLECTION)
...
def blob_bytes(collection: fabriks.Collection) -> dict[tuple[int, int], int]:
...
def summarise(collection: fabriks.Collection, plan: list[fabriks.CellEntry], sizes: dict) -> tuple[int, int, str]:
...
def main() -> None:
    ...
    for factor in (0.05, 0.2, 0.5, 1.0, 2.0, 5.0):
        ...
        camera = fabriks.Camera.perspective(eye, fov_y=0.8, viewport_height=1080)
...
def compare_backends() -> None:
    ...
    for name in (fabriks.SIMPLIFICATION_QUADRIC, fabriks.SIMPLIFICATION_GREEDY):
        with warnings.catch_warnings():
            warnings.simplefilter("ignore")
            built = fabriks.build_collection(
    ...
    print(
        ...
        "\n"
        "    fabriks.build_collection(objects, ..., simplifier='GREEDY')\n"
        "    fabriks.build_collection(objects, ..., simplifier=fabriks.GreedyEdgeCollapse())\n"
        "    fabriks.build_collection(objects, ..., decimation=fabriks.Decimation.half())"
    )
