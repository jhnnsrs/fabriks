...
| --- | --- |
| [`01_trimesh_to_fabriks.py`](01_trimesh_to_fabriks.py) | `{id: trimesh.Trimesh}` → a collection on disk. Both the one-call `write_meshes` form and the two-step build-then-inspect-then-write form, plus what the tree looks like and what each level costs. |
| [`02_fabriks_to_trimesh.py`](02_fabriks_to_trimesh.py) | A collection → `trimesh.Trimesh` objects. One object reassembled across its cells, one object sliced out of a shared cell, and a whole level exported as OBJ. Measures the round-trip error. |
...
single file out of here and run it anywhere fabriks is installed.
...
`fabriks.DirectoryStore` is used throughout so the output is inspectable, but it is the only
line that changes:
