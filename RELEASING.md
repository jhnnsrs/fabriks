...
`fabriks` ships as a PyPI package (`fabriks`). Versioning is automated by
...
| --- | --- | --- |
| `main` | stable `X.Y.Z` | the default install (`pip install fabriks`) |
| `next` | prereleases `X.Y.Z-rc.N` | published as a **prerelease** — only reached via `pip install fabriks --pre` or an exact pin |
| `N.x` (e.g. `2.x`) | maintenance `X.Y.Z` | published stable for an older major |

PyPI marks `…-rc.N` versions as prereleases, so a plain `pip install fabriks`
...
```sh
pip install fabriks --pre          # latest rc (or stable, whichever is newer)
pip install 'fabriks==3.0.0-rc.1'  # pin a specific rc
```

Stable consumers (`pip install fabriks`) are unaffected by the `next` channel.
